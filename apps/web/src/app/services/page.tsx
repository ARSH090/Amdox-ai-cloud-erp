...
export default function ServicesPricingPage() {
  ...
  const calculateTotal = () => {
    const basePrice = plan === "starter" ? 250 : plan === "enterprise" ? 1200 : 0;
    const userCost = users * (plan === "starter" ? 8 : 6);
    const storageCost = (storage / 100) * 15;
    const aiCost = dedicatedForecast ? 350 : 0;
    ...
    <div className="min-h-screen flex flex-col justify-between bg-[#030712]">
      ...
      <nav className="w-full bg-[#030712]/80 border-b border-white/5 h-16 flex items-center">
        <div className="flex justify-between items-center w-full px-8 max-w-7xl mx-auto">
          ...
          <Link href="/" className="text-xs uppercase font-bold text-[#918fa1] hover:text-white transition-colors duration-300">
            Back to Home
          </Link>
        </div>
      </nav>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          {/* Controls - Left Col */}
          <div className="md:col-span-7 bg-white/[0.02] border border-white/5 backdrop-blur-sm p-6 rounded-2xl space-y-6">
            {/* Plan Tier */}
            <div>
              <label className="block text-xs font-mono uppercase text-[#918fa1] mb-3">Platform Tier</label>
              <div className="grid grid-cols-3 gap-3">
                {(["trial", "starter", "enterprise"] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => setPlan(t)}
                    className={`py-2 px-3 rounded-lg text-xs font-bold uppercase transition-all duration-200 border ${
                      plan === t
          ...
          <div className="md:col-span-5 bg-white/[0.02] border border-white/5 backdrop-blur-sm p-6 rounded-2xl border-[#4F46E5]/20 flex flex-col justify-between min-h-[350px]">
            <div>
            ...
            <div className="pt-6 border-t border-white/5 mt-6">
              <div className="flex justify-between items-baseline mb-6">
                ...
                <span className="text-3xl font-extrabold text-[#10B981]">${calculateTotal().toLocaleString()}</span>
              </div>
              <Link
                href="/contact"
                className="w-full block bg-[#4F46E5] text-white py-3 rounded-lg font-bold text-xs uppercase tracking-wider hover:translate-y-[-2px] transition-all duration-200 text-center shadow-lg shadow-[#4F46E5]/20"
              >
