...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen flex bg-[#030712]">
      {/* Sidebar Panel */}
      <aside
        className={`bg-[#080e1a] border-r border-white/5 flex flex-col justify-between py-6 px-4 transition-all duration-300 fixed left-0 top-0 h-full z-30 ${
      ...
      >
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.path}
                  href={link.path}
                  className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-200 font-mono text-xs ${
                    active
        ...
        <div className="border-t border-white/5 pt-4">
          <Link
            href="/"
            className="flex items-center gap-3 p-3 text-xs text-[#918fa1] hover:bg-white/5 hover:text-white rounded-lg transition-all duration-200 font-mono"
          >
