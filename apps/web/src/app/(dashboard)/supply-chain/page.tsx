// apps/web/src/app/(dashboard)/supply-chain/page.tsx
"use client";
...
export default function SupplyChainDashboardPage() {
  const { data: items, isLoading, error } = useQuery<InventoryItem[]>({
    ...
    queryFn: () => fetchFromProductionEngine<InventoryItem[]>("supply-chain/inventory"),
  });
  ...
  return (
    <div className="space-y-8 min-h-screen bg-[#030712] text-white p-8">
      ...
      {/* Warehouse Grid */}
      <div className="bg-white/[0.02] border border-white/5 backdrop-blur-sm p-6 rounded-2xl">
        ...
        {!items || items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-white/5 rounded-xl">
            <span className="material-symbols-outlined text-red-500/20 text-5xl mb-4">database_off</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {items.map((item) => {
              ...
              return (
                <div 
                  key={item.sku}
                  className={`bg-white/[0.02] border border-white/5 backdrop-blur-sm p-6 rounded-xl flex flex-col justify-between min-h-[160px] transition-all duration-200 ${
                    isBelowReorder 
