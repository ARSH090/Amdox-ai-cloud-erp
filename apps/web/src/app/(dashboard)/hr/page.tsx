...
export default function RealTimeHrPayrollDashboard() {
  ...
  const dispatchPayrollRunMutation = useMutation({
    mutationFn: (payrollConfig: Record<string, unknown>) =>
      fetchFromProductionEngine<{ batchTrackingId: string; recordsQueuedCount: number }>("hr/payroll-run", {
        method: "POST",
  ...
  return (
    <main className="p-8 bg-[#030712] min-h-screen text-white space-y-8">
      ...
      {/* BullMQ Task Progress Monitoring Controls */}
      ...
      {/* Active Workforce Database Grid */}
